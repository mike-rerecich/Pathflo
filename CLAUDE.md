...
- **`app/results/page.jsx`** (~2300 lines) — The core engine, all in one file:
  ...
  - The page also renders the dependency graph, calls `POST /api/analyze` to get the AI-generated agent readout, and handles saving/loading projects, a CSV export of the milestone table, and a client-generated PowerPoint export (`exportToPowerPoint`, using `pptxgenjs`) built from the `computeCPM` result plus the Executive Writer's `aiReadout` text — no PDF export actually exists despite this doc previously claiming one did.
