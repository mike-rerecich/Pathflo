...
function Preloader({ onDone }) {
  ...
  return (
    <div style={{
      ...
      opacity: phase === "fade" ? 0 : 1, transition:"opacity 0.7s ease",
      pointerEvents: phase === "fade" ? "none" : "auto",
    }}>
      <div className="pv-pl-scan"/>
      <Logo color={G} size={40}/>
      <div style={{ marginTop:"1rem", fontFamily:"'DM Mono','Courier New',monospace", fontSize:"0.7rem", color:G, letterSpacing:"0.16em" }}>PATHFLO</div>
      <div style={{ marginTop:"0.9rem", width:140, height:2, background:"rgba(255,255,255,0.06)", borderRadius:1, overflow:"hidden" }}>
        <div className="pv-pl-bar" style={{ height:"100%", background:G, borderRadius:1 }}/>
      </div>
    </div>
  );
}

/* ─── BackgroundField canvas ─────────────────────────────────── */
function BackgroundField() {
  ...
  useEffect(() => {
    ...
    const systems = Array.from({ length: 6 }, () => ({
      ...
      y: Math.random() * (H || 900),
      burst: Math.random() * 300,
      orbs: Array.from({ length: 3 + Math.floor(Math.random() * 4) }, (_, j) => ({
    ...
    const mesh = Array.from({ length: 22 }, () => ({
      x: Math.random() * (W || 1400),
      y: Math.random() * (H || 900),
    ...
    const draw = () => {
      ...
      for (let i = 0; i < mesh.length; i++) {
        ...
        for (let j = i + 1; j < mesh.length; j++) {
          ...
          if (d < 220) {
            ...
            ctx.strokeStyle = `rgba(62,203,111,${(1 - d / 220) * 0.055})`;
      ...
      systems.forEach(s => {
        s.burst++;
        const bursting = s.burst % 280 < 24;
        const grd = ctx.createRadialGradient(s.x, s.y, 0, s.x, s.y, 60);
        grd.addColorStop(0, `rgba(62,203,111,${bursting ? 0.065 : 0.028})`);
        grd.addColorStop(1, "transparent");
        ctx.beginPath(); ctx.arc(s.x, s.y, 60, 0, Math.PI * 2);
        ctx.fillStyle = grd; ctx.fill();
        s.orbs.forEach(o => {
          o.angle += o.speed;
          const px = s.x + Math.cos(o.angle) * o.r;
          const py = s.y + Math.sin(o.angle) * o.r;
          ctx.beginPath(); ctx.arc(px, py, o.size * (bursting ? 1.6 : 1), 0, Math.PI * 2);
          ctx.fillStyle = `rgba(62,203,111,${o.alpha * (bursting ? 1.4 : 1)})`;
          ctx.fill();
        });
        const outerR = s.orbs[s.orbs.length - 1]?.r || 40;
        ctx.beginPath(); ctx.arc(s.x, s.y, outerR, 0, Math.PI * 2);
        ctx.strokeStyle = "rgba(62,203,111,0.025)"; ctx.lineWidth = 1; ctx.stroke();
      });
  ...
  return <canvas ref={canvasRef} style={{ position:"fixed", inset:0, width:"100%", height:"100%", zIndex:0, pointerEvents:"none", opacity:0.65 }}/>;
}
  ...
  return (
    <div style={{ overflowX:"auto", WebkitOverflowScrolling:"touch", width:"100%" }}>
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display:"block", minWidth:W }}>
        <defs>
          <marker id="pv-arr-c" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill={C.crit} opacity="0.85"/></marker>
          <marker id="pv-arr-n" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill={C.textSub} opacity="0.5"/></marker>
          <marker id="pv-arr-u" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill={C.blue} opacity="0.9"/></marker>
          <marker id="pv-arr-d" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill={C.crit} opacity="0.7"/></marker>
          <marker id="pv-arr-dim" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill={C.textSub} opacity="0.2"/></marker>
          <filter id="pv-glow" x="-30%" y="-30%" width="160%" height="160%"><feGaussianBlur stdDeviation="3" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        </defs>
        {cascadeRect && (
          <g>
        ...
        {tasks.map(t => t.predecessors.map(pid => {
          ...
          const marker=isDim?"url(#pv-arr-dim)":isUp?"url(#pv-arr-u)":isDown?"url(#pv-arr-d)":isCritE?"url(#pv-arr-c)":"url(#pv-arr-n)";
...
function TerminalWindow() {
  ...
  const lines = [
    { text: '$ pathflo analyze "Website Launch"',       color: G },
    ...
    { text: '  ✓  On-time confidence: 86%',            color: G },
    { text: '  ✓  Executive report ready to share',    color: G },
  ];
  useEffect(() => {
  ...
  return (
    <div style={{ background:"#0D1117", border:"1px solid #1C2128", borderRadius:14, overflow:"hidden", boxShadow:`0 32px 80px rgba(0,0,0,0.6),0 0 0 1px rgba(62,203,111,0.06),inset 0 1px 0 rgba(255,255,255,0.03)` }}>
      <div style={{ display:"flex", alignItems:"center", gap:6, padding:"11px 16px", background:"#161B22", borderBottom:"1px solid #1C2128" }}>
        ...
        <span style={{ marginLeft:10, fontSize:"0.7rem", color:"#484F58", fontFamily:"system-ui", letterSpacing:"0.04em" }}>pathflo — analysis</span>
      </div>
      <div style={{ padding:"20px 22px", minHeight:192 }}>
        {lines.slice(0, visible).map((line, i) => (
          <div key={i} style={{ color:line.color, lineHeight:1.85, fontFamily:"'DM Mono','Courier New',monospace", fontSize:"0.8rem", animation:"pv-termLine 0.18s ease both" }}>{line.text}</div>
        ))}
        {visible > 0 && visible < lines.length && (
          <span style={{ color:G, fontFamily:"monospace", animation:"pv-blink 1s step-end infinite" }}>▌</span>
        )}
      </div>
    </div>
  );
...
function ConfidenceWidget() {
  ...
  return (
    <div style={{ background:"rgba(13,17,23,0.9)", border:"1px solid #1C2128", borderRadius:12, padding:"1rem 1.25rem", backdropFilter:"blur(12px)", boxShadow:"0 8px 32px rgba(0,0,0,0.4)", display:"flex", flexDirection:"column", gap:"0.6rem" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <span style={{ fontSize:"0.68rem", fontWeight:700, letterSpacing:"0.08em", color:"#484F58", textTransform:"uppercase" }}>ON-TIME CONFIDENCE</span>
        <span style={{ fontSize:"1.1rem", fontWeight:700, color:G, fontFamily:"'Fraunces',serif" }}>{pct}%</span>
      </div>
      <div style={{ height:4, borderRadius:2, background:"#1C2128", overflow:"hidden" }}>
        <div style={{ height:"100%", borderRadius:2, background:`linear-gradient(90deg,${G},#22C55E)`, width:`${pct}%`, transition:"width 1.4s cubic-bezier(0.34,1.56,0.64,1)", boxShadow:"0 0 8px rgba(62,203,111,0.5)" }}/>
      </div>
      <div style={{ display:"flex", justifyContent:"space-between", fontSize:"0.72rem" }}>
        <span style={{ color:"#EF4444" }}>⚠ Dev bottleneck</span>
...
export default function PreviewPage() {
  ...
  const agents = [
    ...
    { n:"02", name:"Fix Generator", tier:"Free", color:G, plain:"Takes the risks and produces specific actions to fix them. Not generic advice — it references your actual tasks and owners." },
    { n:"03", name:"Cascade Modeler", tier:"Free", color:G, plain:"Picks the highest-risk failure point and plays it out step by step. If Task A slips 5 days, here's exactly which tasks get blocked, in what order, and what the final deadline impact is." },
    { n:"04", name:"Executive Writer", tier:"Free", color:G, plain:"Turns everything above into a clean, one-page project status summary you can read in 60 seconds. Verdict, top risks, recommended actions, and a bottom line — ready to share." },
    ...
    { n:"06", name:"Deadline Reverse-Engineer", tier:"Team", color:T.amber, plain:"The deadline is fixed. This agent works backwards from it and tells you exactly what has to be cut, compressed, or resourced to hit it. Three options ranked by realism." },
  ];
    ...
    <main style={{ background:T.bg, color:T.text, fontFamily:"'DM Sans',system-ui,sans-serif", overflowX:"hidden" }}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes pv-termLine{from{opacity:0;transform:translateX(-4px)}to{opacity:1;transform:translateX(0)}}
        ...
        @keyframes pv-emberRise{0%{transform:translateY(0) scale(1);opacity:0.7}60%{opacity:0.5}100%{transform:translateY(-130px) scale(0.3);opacity:0}}
        @keyframes pv-cometStreak{0%{transform:translateX(-100px) translateY(0);opacity:0}10%{opacity:0.7}90%{opacity:0.5}100%{transform:translateX(320px) translateY(-65px);opacity:0}}
        ...
        @keyframes pv-drift{0%,100%{transform:translateY(0) translateX(0) scale(1)}33%{transform:translateY(-18px) translateX(10px) scale(1.04)}66%{transform:translateY(10px) translateX(-8px) scale(0.97)}}
        @keyframes pv-drift2{0%,100%{transform:translateY(0) translateX(0)}40%{transform:translateY(14px) translateX(-12px)}70%{transform:translateY(-10px) translateX(8px)}}
        @keyframes pv-scanShoot{0%{top:-4px;opacity:0.8}100%{top:100%;opacity:0}}
        @keyframes pv-barGrow{from{width:0}to{width:100%}}
        @keyframes pv-noiseShift{0%{background-position:0 0}100%{background-position:100px 100px}}

        .pv-pl-scan{position:absolute;inset:0;overflow:hidden}
        .pv-pl-scan::after{content:'';position:absolute;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,${G}80,transparent);animation:pv-scanShoot 1.1s ease-in-out infinite}
        .pv-pl-bar{animation:pv-barGrow 1.3s ease forwards}

        .pv-grain{position:fixed;inset:0;z-index:5;pointer-events:none;opacity:0.022;
        ...
        .pv-comet{position:absolute;width:80px;height:1px;background:linear-gradient(90deg,transparent,${G}90,transparent);border-radius:1px;animation:pv-cometStreak linear infinite;pointer-events:none}
        .pv-halo{position:absolute;border-radius:50%;border:1px solid rgba(62,203,111,0.07);animation:pv-haloSpin 28s linear infinite;pointer-events:none}
        .pv-halo2{position:absolute;border-radius:50%;border:1px solid rgba(62,203,111,0.04);animation:pv-haloSpinR 44s linear infinite;pointer-events:none}
        .pv-mist{position:absolute;border-radius:50%;filter:blur(80px);pointer-events:none}
        ...
        .pv-ham{display:flex;flex-direction:column;gap:5px;cursor:pointer;padding:4px;border:none;background:transparent;z-index:201;position:relative}
        ...
        .pv-nav-panel{position:fixed;inset:0;background:rgba(8,10,8,0.97);z-index:199;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2.5rem;opacity:0;pointer-events:none;transition:opacity 0.25s}
        .pv-nav-panel.open{opacity:1;pointer-events:auto}
        .pv-nav-panel a{font-size:clamp(1.4rem,4vw,2rem);font-weight:300;color:#8A9E8A;text-decoration:none;transition:color 0.15s;letter-spacing:-0.02em;font-family:'Fraunces',serif}
        .pv-nav-panel a:hover{color:#EEF2EE}

        .pv-comp-row:hover .pv-comp-right{background:rgba(62,203,111,0.05)}
        .pv-terminal-float{animation:pv-floatUp 5s ease-in-out infinite}
        ...
        @media(max-width:900px){
          ...
          .pv-hero-stats{gap:2rem!important;flex-wrap:wrap!important;justify-content:center!important}
          .pv-comp-cell{padding:0.75rem 1rem!important;font-size:0.8rem!important}
        }
      `}</style>
      ...
      {/* Nav overlay panel */}
      <div className={`pv-nav-panel ${navOpen ? "open" : ""}`} onClick={() => setNavOpen(false)}>
        ...
        <a href="/app" className="pv-btn-p" style={{ fontSize:"1.1rem", padding:"0.9rem 2.5rem", marginTop:"0.5rem" }} onClick={() => setNavOpen(false)}>Get started →</a>
      </div>

      {/* Nav bar */}
      <nav style={{
        position:"fixed", top:2, left:0, right:0, zIndex:200, height:62,
        ...
        <a href="/preview" style={{ textDecoration:"none", display:"flex", alignItems:"center", gap:"0.5rem", position:"absolute", left:"50%", transform:"translateX(-50%)" }}>
      ...
      <section style={{ position:"relative", zIndex:1, padding:"148px 2rem 110px", maxWidth:1200, margin:"0 auto", overflow:"hidden" }}>
        {/* Mist orbs */}
        <div className="pv-mist" style={{ width:520, height:520, background:"radial-gradient(circle,rgba(62,203,111,0.09) 0%,transparent 70%)", top:"-10%", left:"55%", animation:"pv-drift 22s ease-in-out infinite" }}/>
        <div className="pv-mist" style={{ width:360, height:360, background:"radial-gradient(circle,rgba(62,203,111,0.06) 0%,transparent 70%)", top:"20%", left:"-5%", animation:"pv-drift2 18s ease-in-out infinite" }}/>
        <div className="pv-mist" style={{ width:280, height:280, background:"radial-gradient(circle,rgba(59,130,246,0.04) 0%,transparent 70%)", top:"60%", right:"0%", animation:"pv-drift 26s ease-in-out infinite reverse" }}/>
        {/* Halos */}
        <div className="pv-halo" style={{ width:560, height:560, top:"calc(50% - 280px)", left:"calc(50% - 280px)" }}/>
        <div className="pv-halo2" style={{ width:760, height:760, top:"calc(50% - 380px)", left:"calc(50% - 380px)" }}/>
        {/* Embers */}
        {[
        ...
        {/* Comets */}
        <div className="pv-comet" style={{ top:"22%", left:"60%", animationDuration:"6s", animationDelay:"0s" }}/>
        <div className="pv-comet" style={{ top:"55%", left:"20%", animationDuration:"8s", animationDelay:"3s" }}/>
        ...
        <div className="pv-hero-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"4rem", alignItems:"center" }}>
          <div>
            ...
            <div className="pv-h5 pv-hero-stats" style={{ display:"flex", gap:"3rem" }}>
          ...
          <div className="pv-hero-right pv-terminal-float" style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
            <TerminalWindow/>
            <ConfidenceWidget/>
          </div>
        </div>
      ...
      <RevealSection id="graph" style={{ padding:"0 1.5rem", maxWidth:1140, margin:"0 auto 5rem" }}>
        ...
        <div style={{ background:"#0D1117", border:"1px solid #1C2128", borderRadius:20, overflow:"hidden", boxShadow:"0 8px 40px rgba(0,0,0,0.5)" }}>
            ...
            <div className="pv-graph-rp" style={{ borderLeft:"1px solid #1C2128", padding:"1.25rem", display:"flex", flexDirection:"column", gap:"1rem" }}>
              ...
              <div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.4rem" }}>
                  {[{label:"Delay Risk",val:"+5–7d",color:"#EF4444"},{label:"Tasks Blocked",val:"3 tasks",color:"#EF4444"},{label:"Owners at Risk",val:"3 people",color:"#F59E0B"},{label:"Cost Exposure",val:"$4,200",color:"#F59E0B"}].map((s,i)=>(
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:"1rem", padding:"0.75rem 1.5rem", borderTop:"1px solid #1C2128" }}>
            {[{color:"#EF4444",label:"Critical / Zero float"},{color:"#F59E0B",label:"At risk"},{color:G,label:"On track"},{color:"#3E4E3E",label:"Has buffer"}].map((l,i)=>(
        ...
        <div style={{ background:"#0D1117", border:"1px solid #1C2128", borderRadius:16, overflow:"hidden" }}>
          ...
          {comparison.map(({pm,pathflo},i)=>(
            <div key={i} className="pv-comp-row" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", borderTop:"1px solid #1C2128" }}>
              <div className="pv-comp-cell" style={{ padding:"0.9rem 1.5rem", fontSize:"0.875rem", color:"#8A9E8A", display:"flex", gap:"0.6rem", alignItems:"flex-start" }}>
                <span style={{ color:"#EF4444", flexShrink:0, fontSize:"0.68rem", marginTop:"0.1rem" }}>✕</span>{pm}
              </div>
              <div className="pv-comp-cell pv-comp-right" style={{ padding:"0.9rem 1.5rem", fontSize:"0.875rem", color:"#EEF2EE", display:"flex", gap:"0.6rem", alignItems:"flex-start", transition:"background 0.2s" }}>
            ...
            <div key={i} className="pv-card">
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"0.65rem" }}>
                ...
                <div style={{ fontSize:"0.55rem", fontWeight:700, padding:"0.2rem 0.5rem", borderRadius:100,
                  ...
                  color:a.tier==="Free"?G:a.tier==="Solo"?"#A78BFA":"#F59E0B",
                  border:`1px solid ${a.tier==="Free"?G+"40":a.tier==="Solo"?"#A78BFA40":"#F59E0B40"}`,
                }}>{a.tier}</div>
              </div>
      ...
      <RevealSection style={{ padding:"0 1.5rem", maxWidth:620, margin:"0 auto 5rem" }}>
        <div style={{ textAlign:"center", padding:"3.5rem 2rem", background:"#0D1117", border:"1px solid #252D25", borderRadius:24, boxShadow:"0 0 0 1px rgba(62,203,111,0.06),0 32px 80px rgba(0,0,0,0.4)", position:"relative", overflow:"hidden" }}>
          <div className="pv-mist" style={{ width:300, height:300, background:"radial-gradient(circle,rgba(62,203,111,0.07) 0%,transparent 70%)", top:"-30%", left:"50%", transform:"translateX(-50%)", animation:"pv-drift 18s ease-in-out infinite" }}/>
      ...
      {/* Preview banner */}
      <div style={{ position:"fixed", bottom:16, left:"50%", transform:"translateX(-50%)", zIndex:9000, background:"rgba(8,10,8,0.96)", border:"1px solid #252D25", borderRadius:100, padding:"0.5rem 1.25rem", display:"flex", alignItems:"center", gap:"0.75rem", backdropFilter:"blur(16px)", boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
        <span style={{ width:6, height:6, borderRadius:"50%", background:G, animation:"pv-pulse 2s infinite" }}/>
        <span style={{ fontSize:"0.72rem", color:"#8A9E8A", fontFamily:"'DM Mono',monospace" }}>PREVIEW — not the live site</span>
        <a href="/" style={{ fontSize:"0.72rem", color:G, textDecoration:"none", fontWeight:600 }}>← Back to live site</a>
      </div>
    </main>
  );
}
