...
function ResultsContent() {
  ...
  async function handleSendMagicLink() {
    ...
    setSaveStatus(error ? "error" : "sent");
  }
  ...
  async function handleSaveProject() {
    ...
    const body = { projectData: data, name: data.name, ...(savedProjectId ? { projectId: savedProjectId } : {}) };
    ...
    if (json.project) {
      setSavedProjectId(json.project.id);
      setSaveStatus("saved");
      setTimeout(() => { setSaveModal(false); setSaveStatus(null); }, 1800);
    } else {
      setSaveStatus("error");
    }
  }
  ...
  const SaveModal = () => (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.72)",zIndex:9000,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}} onClick={()=>{setSaveModal(false);setSaveStatus(null);}}>
      <div style={{background:C.surface,border:"1px solid "+C.border,borderRadius:16,padding:"1.5rem",maxWidth:400,width:"100%",animation:"fadeUp 0.2s ease"}} onClick={e=>e.stopPropagation()}>
        <div style={{fontSize:"0.6rem",color:C.green,fontWeight:700,letterSpacing:"0.12em",marginBottom:"0.65rem"}}>SAVE PROJECT</div>
        <div style={{fontSize:"1rem",fontWeight:700,color:C.text,marginBottom:"0.25rem"}}>{data?.name}</div>
        <div style={{fontSize:"0.78rem",color:C.textMid,marginBottom:"1.25rem"}}>Save this project so you can come back and adjust tasks, dates, or owners — without re-entering everything.</div>

        {!authUser && saveStatus !== "sent" && (
          <>
            <div style={{fontSize:"0.72rem",color:C.textMid,marginBottom:"0.5rem"}}>Enter your email to get a sign-in link:</div>
            <div style={{display:"flex",gap:"0.5rem"}}>
              <input
                ...
                style={{flex:1,background:C.surface2,border:"1px solid "+C.border,borderRadius:8,color:C.text,fontFamily:"inherit",fontSize:"0.85rem",padding:"0.55rem 0.75rem",outline:"none"}}
              />
              <button onClick={handleSendMagicLink} disabled={saveStatus==="sending"} style={{background:C.green,color:"#080A08",border:"none",borderRadius:8,fontFamily:"inherit",fontWeight:700,fontSize:"0.82rem",padding:"0.55rem 1rem",cursor:"pointer",whiteSpace:"nowrap"}}>
                {saveStatus==="sending"?"Sending…":"Send link"}
              </button>
            </div>
            {saveStatus==="error"&&<div style={{fontSize:"0.72rem",color:C.red,marginTop:"0.4rem"}}>Something went wrong. Try again.</div>}
          </>
        )}

        {!authUser && saveStatus === "sent" && (
          <div style={{background:C.greenDim,border:"1px solid "+C.green+"30",borderRadius:10,padding:"1rem",textAlign:"center"}}>
            <div style={{fontSize:"1.2rem",marginBottom:"0.4rem"}}>✉</div>
            <div style={{fontSize:"0.88rem",color:C.green,fontWeight:600,marginBottom:"0.2rem"}}>Check your inbox</div>
            <div style={{fontSize:"0.78rem",color:C.textMid}}>Click the link in the email from Supabase. You'll be signed in and the project will be saved.</div>
          </div>
        )}

        {authUser && saveStatus !== "saved" && (
          <>
            <div style={{fontSize:"0.72rem",color:C.textMid,marginBottom:"0.65rem"}}>Signed in as <strong style={{color:C.text}}>{authUser.email}</strong></div>
            <div style={{display:"flex",gap:"0.5rem"}}>
              <button onClick={handleSaveProject} disabled={saveStatus==="saving"} style={{flex:1,background:C.green,color:"#080A08",border:"none",borderRadius:8,fontFamily:"inherit",fontWeight:700,fontSize:"0.85rem",padding:"0.6rem",cursor:"pointer"}}>
                {saveStatus==="saving"?"Saving…":savedProjectId?"Update saved project":"Save project"}
              </button>
              <button onClick={async()=>{await supabase.auth.signOut();setAuthUser(null);}} style={{background:C.surface2,border:"1px solid "+C.border,borderRadius:8,color:C.textMid,fontFamily:"inherit",fontSize:"0.75rem",padding:"0.6rem 0.75rem",cursor:"pointer"}}>Sign out</button>
            </div>
            {saveStatus==="error"&&<div style={{fontSize:"0.72rem",color:C.red,marginTop:"0.4rem"}}>Save failed. Try again.</div>}
          </>
        )}
        ...
        {saveStatus === "saved" && (
          <div style={{background:C.greenDim,border:"1px solid "+C.green+"30",borderRadius:10,padding:"1rem",textAlign:"center"}}>
            ...
            <div style={{fontSize:"0.88rem",color:C.green,fontWeight:600}}>Project saved</div>
            <div style={{fontSize:"0.75rem",color:C.textMid,marginTop:"0.2rem"}}>You can load it from the start screen anytime.</div>
          </div>
        )}

        <button onClick={()=>{setSaveModal(false);setSaveStatus(null);}} style={{position:"absolute",top:"1rem",right:"1rem",background:"transparent",border:"none",color:C.textDim,cursor:"pointer",fontSize:"1rem",lineHeight:1}}>✕</button>
      </div>
    </div>
  );
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <DependencyGraph
          ...
          onNodeClick={(id) => {
            ...
            if (preview && id) setActiveNav("dependency");
          }}
        />
      </div>
      {selectedNodeId && (
          ...
          {activeNav==="overview" && (
            <div style={{animation:"fadeUp 0.3s ease both"}}>
              ...
              <div style={{...card({border:"1px solid "+verdColor+"50"}),padding:"1.25rem",marginBottom:"0.75rem",position:"relative",overflow:"hidden"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:"0.5rem"}}>
                  {[
                    ...
                    {label:"Recommended Fix",val:result.shuffleOps[0]?`Run "${result.shuffleOps[0].task}" in parallel — saves ~${result.shuffleOps[0].daysSaved}d`:"Validate critical path owners",color:C.green,nav:"optimize"},
              ...
              {result.predictiveRisk?.top3[0] && (
                <div onClick={()=>setActiveNav("cascade")} style={{...card({border:"1px solid "+C.amber+"30",background:C.amberDim}),padding:"1rem",marginBottom:"0.75rem",cursor:"pointer"}}>
              ...
              <div style={{...card(),padding:"0.85rem 1rem"}}>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(80px,1fr))",gap:"0.5rem",textAlign:"center"}}>
                  {[
                    {label:"TASKS",val:result.totalTasks,nav:"dependency"},
                    {label:"CRITICAL PATH",val:result.criticalPath.length,color:C.red,nav:"critical",tip:"Tasks with zero buffer. If any one of these slips, your deadline moves. Tap to see them."},
                    {label:"TEAM",val:result.teamSize},
                    {label:"BUFFER",val:result.bufferDays>=0?result.bufferDays+"d":"Over",color:result.bufferDays>=0?C.green:C.red,nav:"cascade"},
                    {label:"DEPS",val:data.tasks.filter(t=>t.predecessors.length>0).length,nav:"dependency"},
                  ].map((s,i)=>(
